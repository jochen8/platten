                        ...
                        if g.test_outer() and g.test_inner():
                            ...
                            for iy in range(3):
                                for ix in range(3):
                                    ...
                                    if g.slab[ix][iy].typ > 0:
                                        res2.append(R(g.slab[ix][iy].typ, iy * 3 + ix, g.slab[ix][iy].angle))
                            g.result.append(res)
                            g.results.append(res2)
...
if __name__ == '__main__':
    ...
    g.print()
    images.make_garden_image(g, 'ii.png')
    ...
    for x in res:
        ...
        res_x = [cost]
        for n in range(9):
            xxx = xx[n].strip().split(' ')
            wp_typ = int(xxx[0])
            wp_angle = int(xxx[1])
            if wp_typ > 0:
                res_x.append(R(wp_typ, n , wp_angle))
        res2.append(res_x)

    print('len(res2)', len(res2))
    print(res2[:1])
    print(res2[:1][0])
    print(res2[:1][0][1].typ, res2[:1][0][1].n, res2[:1][0][1].angle, )
    print('len(res2)', len(res2))
    #print(res2)
    images.make_garden_image_from_result(res2)
    #print_cost_count()
